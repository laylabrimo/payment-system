    ...
    {loading &&  <Spinner style={{
     ...
     <TextInput editable={!loading} selectTextOnFocus={!loading} onChange={(e)=>{
      ...
      if (e.nativeEvent.text.length>9){
         Keyboard.dismiss()
         findatouser_account(e.nativeEvent.text)

       }
      
     }} keyboardType='numeric' style={styles.input} placeholder='Account Number'/>
     
     <TextInput  editable={!loading} selectTextOnFocus={!loading} defaultValue={touser?touser?.name:''}  disableFullscreenUI style={styles.input} placeholder={loading?'retriving user ...':'Account Name'}/>
     ...
     <Button disabled={!ok} title='Send Now' onPress={()=>{
       setconfirmopen(true)
